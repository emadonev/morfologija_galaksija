...
class VisionTransformer(nn.Module):
  ...
  def forward(self, x, coarse_label=None, ch_out=False):
    B = x.shape[0]
    # Assume x is a standard RGB image with 3 channels.
    ...
    if coarse_label is not None:
        ...
        # Fuse the coarse embedding with each patch token.
        fused = torch.cat([rgb, coarse_emb_expanded], dim=-1)  # shape: [B, num_tokens, 2*embed_dim]
        rgb = self.merge_coarse(fused)  # Assuming merge_lin is redefined for 2*embed_dim -> embed_dim

    # If using a class token, prepend it. Note that the CLS token will now also be influenced indirectly
    # because the other tokens contain the coarse label info.
    if self.cls_token:
        cls_token = einops.repeat(self.cls_token_embed, '1 1 e -> b 1 e', b=B).contiguous()
        rgb = torch.cat([cls_token, rgb], dim=1)
    
    # Pass the tokens (now enriched with coarse label info) through the transformer layers.
    rgb = self.layers(rgb)
    
    if not ch_out:
        if self.cls_token:
            rgb = rgb[:, 1:, :]  # Remove CLS token if not needed later
...
class CvT(nn.Module):
  ...
  def forward(self, x, coarse_label=None):
    # x is your RGB image input of shape [B, 3, H, W]
    x = self.stage1(x)  # Stage 1 processes the image (you can treat it as extracting coarse features)
    x = self.stage2(x)
    
    # For stage3, pass the coarse label as aux_layer for early fusion.
    if self.hint and (coarse_label is not None):
         x = self.stage3(x, coarse_label=coarse_label, ch_out=True)
    else:
         x = self.stage3(x, coarse_label=None, ch_out=True)
    
    # Extract the CLS token (or do further processing as needed)
    x = x[:, 0, :]
    x = self.ff(x)
    return x
...
class TwoStageCvT(nn.Module):
    ...
    def __init__(self, embed_dim, coarse_classes=3, fine_classes=7):
        ...
        self.coarse_model = CvT(embed_dim, coarse_classes, hint=True)
        
        # Second model for fine classification that takes coarse predictions
        self.fine_model = CvT(embed_dim, fine_classes, hint=True, use_coarse_labels=True, num_coarse_classes=coarse_classes)
